...
def load_data():
    ...
    # Solo se usan dos archivos: CSV de tiendas (datos reales) y Excel de objetivos
    ...
    # ── CSV: TIENDAS (tabla más granular — fuente principal) ─────────────────
    # Una fila por tienda × división. Tiene todas las dimensiones: zona, región,
    # DM, AM, tienda, división. Es la base de todos los KPIs y filtros.
    tiendas_raw = pd.read_csv('/tmp/tiendas.csv', encoding='latin-1', dtype=str)
    tiendas_raw.columns = [
        'Unidad_Negocio', 'Zona', 'Region', 'District Manager', 'Area Manager',
        'Cod Tienda', 'Tienda', 'Cod Division', 'Desc Division',
        'Ventas Real', 'Ventas Historico',
        'Quiebra Real', 'Quiebra Pct Real',
        'Quiebra Historico', 'Quiebra Pct Historico',
        'Dif Quiebra PP', 'Delta Quiebra'
    ]
    # Filtrar solo unidades válidas y excluir subtotales
    unidades_validas = ['Ara', 'BdC', 'Ara Franquicia']
    tiendas_raw = tiendas_raw[tiendas_raw['Unidad_Negocio'].isin(unidades_validas)]
    tiendas_raw = tiendas_raw[~tiendas_raw['Zona'].isin(['NO ASIGNADA','Total Ara sin BDC','Total Ara sin BDC y FRA'])]
    tiendas_raw = tiendas_raw[tiendas_raw['Tienda'].notna() & tiendas_raw['Desc Division'].notna()]

    # Limpiar números (vienen con comas: "1,597" → 1597.0)
    def clean_num(v):
    ...
    tiendas_raw['Ventas Real']           = tiendas_raw['Ventas Real'].apply(clean_num)
    tiendas_raw['Ventas Historico']      = tiendas_raw['Ventas Historico'].apply(clean_num)
    tiendas_raw['Quiebra Real']          = tiendas_raw['Quiebra Real'].apply(clean_num)
    tiendas_raw['Quiebra Historico']     = tiendas_raw['Quiebra Historico'].apply(clean_num)
    tiendas_raw['Quiebra Pct Real']      = tiendas_raw['Quiebra Pct Real'].apply(clean_pct)
    tiendas_raw['Quiebra Pct Historico'] = tiendas_raw['Quiebra Pct Historico'].apply(clean_pct)
    tiendas_raw['Dif Quiebra PP']        = tiendas_raw['Dif Quiebra PP'].apply(clean_num)
    tiendas_raw['Region']                = tiendas_raw['Region'].apply(lambda x: str(x).zfill(2) if pd.notna(x) and str(x).strip().isdigit() else x)

    # Tabla de tiendas (agrupada por tienda, sin división)
    tiendas = tiendas_raw.groupby(
        ['Unidad_Negocio','Zona','Region','District Manager','Area Manager','Cod Tienda','Tienda'],
        as_index=False
    ).agg({'Ventas Real':'sum','Ventas Historico':'sum','Quiebra Real':'sum','Quiebra Historico':'sum','Dif Quiebra PP':'sum'})
    tiendas['Quiebra Pct Real']      = tiendas['Quiebra Real'] / tiendas['Ventas Real']
    tiendas['Quiebra Pct Historico'] = tiendas['Quiebra Historico'] / tiendas['Ventas Historico']
    ...
    divisiones = divisiones.rename(columns={'Cod Division':'Cod Division','Desc Division':'Desc Division'})

    # Tabla de regiones (agrupada por zona + región)
    regiones = tiendas_raw.groupby(['Zona','Region'], as_index=False).agg(
        {'Ventas Real':'sum','Ventas Historico':'sum','Quiebra Real':'sum','Quiebra Historico':'sum','Dif Quiebra PP':'sum'}
    )
    regiones['Quiebra Pct Real']      = regiones['Quiebra Real'] / regiones['Ventas Real']
    regiones['Quiebra Pct Historico'] = regiones['Quiebra Historico'] / regiones['Ventas Historico']

    # Tabla de DMs (agrupada por zona + región + DM)
    dm = tiendas_raw.groupby(['Zona','Region','District Manager'], as_index=False).agg(
        {'Ventas Real':'sum','Ventas Historico':'sum','Quiebra Real':'sum','Quiebra Historico':'sum','Dif Quiebra PP':'sum'}
    )
    dm['Quiebra Pct Real']      = dm['Quiebra Real'] / dm['Ventas Real']
    dm['Quiebra Pct Historico'] = dm['Quiebra Historico'] / dm['Ventas Historico']

    # Tabla de AM
    am = tiendas_raw.groupby(['Zona','Region','District Manager','Area Manager'], as_index=False).agg(
        {'Ventas Real':'sum','Ventas Historico':'sum','Quiebra Real':'sum','Quiebra Historico':'sum'}
    )

    # AM ya viene agregado desde tiendas_raw arriba
